"""
NotebookLM CLI wrapper with retry logic and structured output.

Handles:
  - notebook creation / selection
  - source upload with deduplication
  - question asking with retry
...
def create_notebook(title: str) -> str:
    ...
    # Parse ID from output (last non-empty line typically contains the ID)
...
def find_notebook(title_substring: str) -> Optional[str]:
...
def source_exists(filename: str) -> bool:
    """Check if a source file already exists."""
    sources = list_sources()
    for src in sources:
        if filename.lower() in src.get("name", "").lower():
            return True
    return False


def add_source(file_path: str) -> None:
...
def upload_sources_dir(dir_path: Path, pattern: str = "*.md") -> tuple[int, int]:
    """
...
def verify_upload(expected_count: int) -> bool:
    """Verify that expected number of sources are ready."""
    sources = list_sources()
...
def ask(question: str, timeout: int = 180) -> str:
    """Ask current notebook a question, return the answer text."""
    stdout = _retry_run(["ask", question], timeout=timeout, max_retries=2)
    return stdout


def configure_persona(persona: str) -> None:
